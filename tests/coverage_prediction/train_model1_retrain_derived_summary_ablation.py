"""
Model 1 derived-summary-feature ablation retraining.

This script keeps the existing Model 1 artifacts untouched. It trains two
variants into models/model1_retrain_derived_summary:
    - with_derived_summary: coordinate-free baseline
    - remove_derived_summary: removes summary/lag-style RF features

The goal is to test whether the model is leaning on high-level RF summaries
instead of more fundamental physical features.
"""
...
BASE_METADATA = Path("models") / "model1_retrain" / "remove_coordinates" / "pred_rsrp" / "metadata.json"
DATASET_CSV = Path("data") / "model1_coverage_training.csv"
RETRAIN_ROOT = Path("models") / "model1_retrain_derived_summary"
SUMMARY_JSON = RETRAIN_ROOT / "derived_summary_ablation_summary.json"
SUMMARY_CSV = RETRAIN_ROOT / "derived_summary_ablation_metrics.csv"
SHAP_COMPARE_PNG = RETRAIN_ROOT / "derived_summary_shap_before_after.png"
...
def setup_logging() -> logging.Logger:
    ...
    logger = logging.getLogger("model1_derived_summary_ablation")
...
def load_base_features() -> tuple[list[str], list[str]]:
    if not BASE_METADATA.exists():
        raise FileNotFoundError(f"Missing coordinate-free metadata: {BASE_METADATA}")
    metadata = json.loads(BASE_METADATA.read_text(encoding="utf-8"))
    return list(metadata["features"]["numeric"]), list(metadata["features"]["categorical"])


def variant_features(remove_derived_summary: bool) -> tuple[list[str], list[str]]:
    numeric, categorical = load_base_features()
    if remove_derived_summary:
        numeric = [feature for feature in numeric if feature not in DERIVED_SUMMARY_FEATURES]
...
def shap_summary(target_dir: Path, feature_pattern: str) -> dict[str, object]:
    shap_path = target_dir / "shap_importance.csv"
    if not shap_path.exists():
        return {"available": False, "rank": None, "mean_abs_shap": None}
    shap_df = pd.read_csv(shap_path)
    rows = shap_df[shap_df["feature"].astype(str).str.contains(feature_pattern, na=False)].copy()
    if rows.empty:
        return {"available": True, "rank": None, "mean_abs_shap": 0.0}
    rows["rank"] = rows.index + 1
    return {
        "available": True,
        "rank": int(rows["rank"].min()),
        "mean_abs_shap": float(rows["mean_abs_shap"].sum()),
        "rows": rows[["feature", "mean_abs_shap", "rank"]].to_dict(orient="records"),
    }
...
def create_before_after_shap_image() -> Path:
    ...
    before_root = RETRAIN_ROOT / "with_derived_summary"
    after_root = RETRAIN_ROOT / "remove_derived_summary"
    ...
    def _family_for_feature(name: str) -> str:
        ...
        if clean.startswith("prev") or clean == "bucket_seq":
            return "Temporal"
        if clean in {"clutter_class", "dominant_band_class"}:
            return "RF"
        if clean in {"morphology_cluster", "terrain_elevation_m", "terrain_slope_deg", "grid_size_m", "grid_area_m2",
                     "road_length_m", "road_density", "green_ratio", "water_ratio", "building_count",
                     "building_area_ratio", "avg_building_area_m2", "park_open_area", "open_area_ratio",
                     "mall_presence", "metro_presence", "clutter_transition_flag", "clutter_upgrade_score"}:
            return "Geo"
        return "RF"

    fig, axes = plt.subplots(len(base.TARGETS), 2, figsize=(18, 18), constrained_layout=True)
    panel_titles = ["Before: with derived summary", "After: removed derived summary"]
    ...
    fig.suptitle("Model 1 Derived-Summary Ablation: SHAP Before vs After", fontsize=18, fontweight="bold")
...
def read_target_result(variant_root: Path, target: str) -> dict[str, object]:
    ...
    return {
        ...
        "derived_summary_shap": shap_summary(target_dir, r"pred_.*std|prev_obs_|prev2_obs_|prev_trend_"),
    }


def read_variant_result(variant_name: str, remove_derived_summary: bool) -> dict[str, object]:
    numeric_features, categorical_features = variant_features(remove_derived_summary)
    variant_root = RETRAIN_ROOT / variant_name
    return {
        "variant": variant_name,
        "remove_derived_summary": remove_derived_summary,
        "removed_features": sorted(DERIVED_SUMMARY_FEATURES) if remove_derived_summary else [],
...
def train_variant(
    ...
    log.info("=" * 70)
    log.info(
        "VARIANT: %s | features=%d | removed=%s",
        variant_name,
        len(numeric_features) + len(categorical_features),
        sorted(DERIVED_SUMMARY_FEATURES) if remove_derived_summary else [],
    )
    log.info("=" * 70)
...
def build_comparison(with_derived: dict[str, object], without_derived: dict[str, object]) -> tuple[dict[str, object], pd.DataFrame]:
    rows = []
    comparison: dict[str, object] = {}
    with_targets = with_derived.get("targets", {})
    without_targets = without_derived.get("targets", {})
                ...
                rows.append(
                    {
                        ...
                        "split": split,
                        "metric": metric,
                        "with_derived_summary": with_value,
                        "remove_derived_summary": without_value,
                        "delta_remove_minus_with": delta,
                    }
                )
                target_summary[f"{split}_{metric}_delta_remove_minus_with"] = delta

        with_shap = (with_targets.get(target, {}) or {}).get("derived_summary_shap", {})
        without_shap = (without_targets.get(target, {}) or {}).get("derived_summary_shap", {})
        target_summary["with_derived_rank"] = with_shap.get("rank")
        target_summary["with_derived_mean_abs_shap"] = with_shap.get("mean_abs_shap")
        target_summary["remove_derived_rank"] = without_shap.get("rank")
        target_summary["remove_derived_mean_abs_shap"] = without_shap.get("mean_abs_shap")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retrain coordinate-free Model 1 with/without derived RF summary features.")
...
def main() -> None:
    ...
    try:
        log.info("Model 1 derived-summary ablation started at %s", datetime.utcnow().isoformat() + "Z")
        log.info("Output root: %s", RETRAIN_ROOT.resolve())

        if args.summarize_only:
            with_derived = read_variant_result("with_derived_summary", False)
            without_derived = read_variant_result("remove_derived_summary", True)
        else:
            df = load_dataset()
            ...
            without_derived = train_variant(
                "remove_derived_summary",
                True,
                train_df,
                valid_df,
                test_df,
        ...
        shap_compare_path = create_before_after_shap_image()
        comparison, comparison_df = build_comparison(with_derived, without_derived)
        ...
        summary = {
            ...
            "removed_features": sorted(DERIVED_SUMMARY_FEATURES),
            "variants": {
                "with_derived_summary": with_derived,
                "remove_derived_summary": without_derived,
            },
            "comparison": comparison,
            "comparison_csv": str(SUMMARY_CSV),
            "shap_compare_png": str(shap_compare_path),
            "interpretation_rule": "For MAE/RMSE, positive delta means removing derived summary features is worse. For R2, negative delta means removing them is worse.",
        }
