...
PRED_COL = "pred_rsrq"
AVG_COL = "avg_rsrq"
BASELINE_AVG_COL = "baseline_avg_rsrq"
BAD_COL = "Bad RSRQ"
...
def _core_config(config) -> core_tilt.TiltRecommendationTestConfig:
    return core_tilt.TiltRecommendationTestConfig(
        ...
        rsrq_threshold=_active_threshold(config),
        sinr_threshold=DISABLED_THRESHOLD,
...
def _filter_bad_samples_active(log_df: pd.DataFrame, allowed_techs) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    work["Bad RSRQ"] = active_bad
    work["Bad SINR"] = False
    bad_df = work.loc[work["Bad RSRQ"]].copy()
    if bad_df.empty:
...
def _aggregate_grid_metrics_active(pred_df: pd.DataFrame, threshold: float) -> pd.DataFrame:
    ...
    out["is_bad_rsrq"] = pd.to_numeric(out[AVG_COL], errors="coerce") < float(threshold)
    out["rsrq_severity"] = (float(threshold) - pd.to_numeric(out[AVG_COL], errors="coerce")).clip(lower=0.0)
    out["is_bad_rsrp"] = out["is_bad_rsrq"]
    out["rsrp_severity"] = out["rsrq_severity"]
    if "avg_rsrp" not in out.columns:
        out["avg_rsrp"] = np.nan
    if "avg_sinr" not in out.columns:
        out["avg_sinr"] = np.nan
    return out
...
def _grid_reference_metrics_active(grid_analytics_df: pd.DataFrame, threshold: float) -> pd.DataFrame:
    ...
    work["avg_sinr"] = pd.to_numeric(work.get("baseline_avg_sinr"), errors="coerce") if "baseline_avg_sinr" in work.columns else np.nan
...
def _build_grid_ranked_summary_active(
    ...
    bad_grid_ids = set(grid_metrics.loc[grid_metrics["is_bad_rsrq"].fillna(False), "grid_id"].tolist())
    if not bad_grid_ids:
        return pd.DataFrame(), grid_metrics
    ...
    summary = (
        ...
        .agg(**{"Bad RSRQ": ("grid_id", "count"), "Bad Grid Count": ("grid_id", "nunique"), "mean_bad_grid_rsrq": (PRED_COL, "mean"), "bad_grid_severity": ("_severity", "sum")})
        .reset_index()
    )
    summary["Bad RSRP"] = summary["Bad RSRQ"]
    summary["Bad SINR"] = 0
    summary["Bad Samples"] = pd.to_numeric(summary["Bad RSRQ"], errors="coerce").fillna(0).astype(int)
    summary["total_bad_samples"] = summary["Bad Samples"]
    return summary.sort_values(["Bad Grid Count", "bad_grid_severity", "Bad RSRQ", "Cell ID"], ascending=[False, False, False, True]).reset_index(drop=True), grid_metrics
...
def _grid_validation_payload_active(
    ...
    payload: Dict[str, object] = {
        ...
        "comparison_operator": "avg_rsrq < threshold",
        ...
        "recomputed_bad_grid_count": int(recomputed["is_bad_rsrq"].fillna(False).sum()) if not recomputed.empty else 0,
    ...
    if reference.empty:
        return payload
    payload["grid_analytics_bad_grid_count"] = int(reference["is_bad_rsrq"].fillna(False).sum())
    if not recomputed.empty:
        compare = recomputed[["grid_id", "avg_rsrq"]].merge(reference[["grid_id", "avg_rsrq"]], on="grid_id", how="inner", suffixes=("_recomputed", "_analytics"))
        payload["common_grid_count"] = int(len(compare))
        if not compare.empty:
            delta = (pd.to_numeric(compare["avg_rsrq_recomputed"], errors="coerce") - pd.to_numeric(compare["avg_rsrq_analytics"], errors="coerce")).abs()
            payload["avg_abs_rsrq_delta_vs_grid_analytics"] = float(delta.mean())
            payload["max_abs_rsrq_delta_vs_grid_analytics"] = float(delta.max())
    return payload
...
def _score_candidate_active(
    ...
    metrics["grid_mean_rsrp_delta_bad_baseline"] = metrics.get("mean_rsrq_delta", 0.0)
    metrics["grid_mean_rsrq_delta_bad_baseline"] = metrics.get("mean_rsrq_delta", 0.0)
    metrics["grid_scoring_source"] = "core_tilt_rsrq_formula_fast_runtime"
...
def _changed_cell_local_metrics_active(
    ...
    for update in updates:
        ...
        rows.append({
            ...
            "before_avg_rsrq": before_avg,
            "after_avg_rsrq": after_avg,
            "avg_rsrq_delta": after_avg - before_avg if pd.notna(before_avg) and pd.notna(after_avg) else np.nan,
            "before_sample_count": int(len(before_values)),
            "after_sample_count": int(len(after_values)),
        })
    if not rows:
        return {"changed_cell_local_metrics": "[]", "changed_cell_bad_sample_reduction_sum": 0.0, "changed_cell_avg_rsrq_delta_mean": np.nan}
    ...
    return {
        ...
        "changed_cell_avg_rsrq_delta_mean": float(pd.to_numeric(local_df["avg_rsrq_delta"], errors="coerce").mean()),
    }
...
def _prepare_scope_export_active(df: pd.DataFrame, threshold: float, stage: str) -> pd.DataFrame:
    ...
    out["is_bad_rsrq"] = out[PRED_COL] < float(threshold)
    out["is_bad_rsrp"] = out["is_bad_rsrq"]
    out["stage"] = stage
    return out
