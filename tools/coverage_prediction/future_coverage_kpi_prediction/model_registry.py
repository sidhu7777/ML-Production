...
MODEL_ROOT = Path(__file__).resolve().parent / "weights" / "model1_current_state_causal_delta_clean_20260805"
TARGETS = ["pred_rsrp", "pred_rsrq", "pred_sinr"]
FORBIDDEN_PRODUCTION_FEATURES = {
    "bucket_seq",
    "time_bucket",
    "current_time_bucket",
    "future_time_bucket",
...
def load_latest_model1_bundle(model_root: Path = MODEL_ROOT) -> Model1Bundle:
    ...
    numeric = list(first_meta.get("numeric_features") or features.get("numeric") or [])
    categorical = list(first_meta.get("categorical_features") or features.get("categorical") or [])
    ...
    prediction_mode = str(first_meta.get("prediction_mode") or "current_state_future_delta")
    model_type = str(first_meta.get("model_type") or "future_coverage_kpi_prediction")
    return Model1Bundle(
        models=models,
        metadata=metadata,
        ...
        model_version=f"{model_type}_{prediction_mode}_{trained_at}".strip("_"),
    )
