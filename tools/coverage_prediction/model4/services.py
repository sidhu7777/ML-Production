...
DEFAULT_MODEL2_INPUT_EXCEL = ML_ROOT / "tools" / "coverage_prediction" / "model2" / "data" / "project_196_model2_demand_capacity_input.xlsx"
DEFAULT_OUTPUT_ROOT = ML_ROOT / "outputs" / "model4_future_recommendation"
...
def _wait_for_job(service: Any, job_id: str, *, timeout_sec: int, label: str) -> dict[str, Any]:
    ...
    while True:
        ...
        if progress != last_progress:
            print(f"[MODEL4][UPSTREAM] {label} {status.get('status')}: {progress}", flush=True)
...
class LTEModel4FutureRecommendationService:
    ...
    def _update(self, job_id: str, status: str, progress: str, **extra: Any) -> None:
        ...
        print(f"[MODEL4][{job_id[:8]}] {status}: {progress}", flush=True)
    ...
    def _run_job(self, job_id: str, cfg: dict[str, Any]) -> None:
        ...
        try:
            ...
            model_run_id = str(cfg.get("model_run_id") or f"model4_{project_id}_{job_id[:8]}")
            ...
            source_model1_run_id = str(cfg.get("source_model1_run_id") or f"{model_run_id}_model1")
            source_model2_run_id = str(cfg.get("source_model2_run_id") or f"{model_run_id}_model2")

            if not bool(cfg.get("skip_model1", False)):
                self._update(job_id, "running", "Running upstream Model 1 coverage forecast")
                from tools.coverage_prediction.model1.services import LTEModel1CoveragePredictionService

                model1_service = LTEModel1CoveragePredictionService()
                ...
                model1_status = _wait_for_job(model1_service, model1_job["job_id"], timeout_sec=int(cfg.get("model1_timeout_sec", 600)), label="Model 1")
                source_model1_run_id = str(model1_status.get("model_run_id") or source_model1_run_id)

            self._update(job_id, "running", "Running upstream Model 2 demand/capacity forecast")
            from tools.coverage_prediction.model2.services import LTEModel2DemandCapacityForecastService
            ...
            model2_status = _wait_for_job(model2_service, model2_job["job_id"], timeout_sec=int(cfg.get("model2_timeout_sec", 600)), label="Model 2")
            source_model2_run_id = str(model2_status.get("model_run_id") or source_model2_run_id)

            self._update(job_id, "running", "Preparing Model 2 future output for Model 4")
            model2_df = db.fetch_model2_results(source_model2_run_id, region=region)
            future_csv = _model2_future_csv(
                model2_df,
                DEFAULT_OUTPUT_ROOT / model_run_id / "model2_future_for_model4.csv",
            )

            self._update(job_id, "running", "Running dynamic Model 4 future recommendations")
            ...
            self._update(job_id, "running", "Saving Model 4 recommendation rows")
            rec_out = db.prepare_recommendation_results(
                ...
                model_version="model4_future_dynamic_rf",
            ...
            self._update(job_id, "running", "Saving Model 4 after-RF rows")
            ...
            self._update(
                job_id,
                "completed",
                "Model 4 future recommendation completed",
                project_id=project_id,
                baseline_job_id=baseline_job_id,
                model_run_id=model_run_id,
                source_model1_run_id=source_model1_run_id,
