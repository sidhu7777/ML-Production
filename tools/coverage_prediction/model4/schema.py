...
RECOMMENDATION_TABLE_NAME = "lte_model4_future_recommendation_results"
RF_SURFACE_TABLE_NAME = "lte_model4_future_recommendation_rf_results"
...
CREATE TABLE IF NOT EXISTS {RECOMMENDATION_TABLE_NAME} (
    ...
    model_name VARCHAR(100) NOT NULL DEFAULT 'model4_future_capacity_recommendation',
    model_version VARCHAR(100) NULL,
    input_source VARCHAR(100) NOT NULL DEFAULT 'model1_model2_future_forecast',
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uq_model4_recommendation (
        project_id,
        baseline_job_id,
        model_run_id,
        recommendation_id
    ),
    KEY idx_model4_project_baseline (project_id, baseline_job_id),
    KEY idx_model4_run (model_run_id),
    KEY idx_model4_sources (source_model1_run_id, source_model2_run_id),
    KEY idx_model4_cell (site_id, sector_id, node_cell_id, band),
    KEY idx_model4_status (status, resolved_flag),
    KEY idx_model4_action (recommended_action, recommendation_level)
...
CREATE TABLE IF NOT EXISTS {RF_SURFACE_TABLE_NAME} (
    ...
    rf_source VARCHAR(100) NOT NULL DEFAULT 'model4_affected_rf_rerun',
    artifact_path VARCHAR(500) NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    PRIMARY KEY (id),
    UNIQUE KEY uq_model4_rf_surface (
        project_id,
        baseline_job_id,
        model_run_id,
        rf_stage,
    ...
    KEY idx_model4_rf_project_baseline (project_id, baseline_job_id),
    KEY idx_model4_rf_run_stage (model_run_id, rf_stage),
    KEY idx_model4_rf_cell (site_id, sector_id, node_cell_id, band),
    KEY idx_model4_rf_grid (grid_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
"""
