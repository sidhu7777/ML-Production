...
def prepare_recommendation_results(
    ...
    *,
    ...
    region: str,
    operator: str | None,
    source_model2_run_id: str | None = None,
    ...
    for idx, row in recommendations.reset_index(drop=True).iterrows():
        before_prb = _safe_num(row.get("prb_before_pct"))
        before_rrc = _safe_num(row.get("rrc_before_pct"))
        ...
        rows.append(
            {
                ...
                "prb_before_pct": before_prb,
                "rrc_before_pct": before_rrc,
                "rrc_users_before": _safe_num(row.get("rrc_users_before")),
                "pressure_before_pct": max([v for v in [before_prb, before_rrc] if v is not None], default=None),
                ...
                "model_name": "current_capacity_recommendation",
                "model_version": model_version,
                "input_source": "current_capacity_input",
        ...
        rows.append(
            {
                ...
                "affected_flag": 1,
                "rf_source": "current_capacity_affected_rf_rerun",
