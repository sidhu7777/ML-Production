...
from .services import CurrentCapacityRecommendationService


current_capacity_recommendation_bp = Blueprint("current_capacity_recommendation", __name__)
lte_model3_current_recommendation_bp = current_capacity_recommendation_bp
service = CurrentCapacityRecommendationService()


@current_capacity_recommendation_bp.route("/run", methods=["POST"])
...
@current_capacity_recommendation_bp.route("/status/<job_id>", methods=["GET"])
...
@current_capacity_recommendation_bp.route("/result/<job_id>", methods=["GET"])
