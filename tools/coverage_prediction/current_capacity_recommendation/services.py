...
DEFAULT_OUTPUT_ROOT = ML_ROOT / "outputs" / "current_capacity_recommendation"
DEFAULT_STABLE_OUTPUT_DIR = ML_ROOT / "models" / "current_capacity_recommendation_experiment"

JOBS: dict[str, dict[str, Any]] = {}


class CurrentCapacityRecommendationService:
    ...
    def _update(self, job_id: str, status: str, progress: str, **extra: Any) -> None:
        ...
        print(f"[CURRENT_CAPACITY_RECOMMENDATION][{job_id[:8]}] {status}: {progress}", flush=True)
    ...
    def _run_job(self, job_id: str, cfg: dict[str, Any]) -> None:
        ...
        try:
            ...
            model_run_id = str(cfg.get("model_run_id") or f"current_capacity_recommendation_{project_id}_{job_id[:8]}")
            ...
            if not input_excel.exists():
                raise FileNotFoundError(f"Current capacity recommendation input Excel not found: {input_excel}")

            self._update(job_id, "running", "Loading current capacity recommendation engine")
            ...
            self._update(job_id, "running", "Running dynamic current capacity recommendations")
            ...
            self._update(job_id, "running", "Saving current capacity recommendation rows")
            rec_out = db.prepare_recommendation_results(
                ...
                model_version="current_capacity_dynamic_rf",
            ...
            self._update(job_id, "running", "Saving current capacity after-RF rows")
            ...
            self._update(
                job_id,
                "completed",
                "Current capacity recommendation completed",
                project_id=project_id,
                baseline_job_id=baseline_job_id,
                model_run_id=model_run_id,
                source_model2_run_id=source_model2_run_id,
...
LTEModel3CurrentRecommendationService = CurrentCapacityRecommendationService
