...
from tools.coverage_prediction.current_capacity_recommendation.schema import (
...
def _drop_obsolete_before_rf_columns(conn) -> None:
    existing = set(_describe(conn, RECOMMENDATION_TABLE_NAME))
    for column in ["before_rf_job_id", "before_rf_rows", "artifact_before_rf_path"]:
        if column in existing:
            conn.execute(text(f"ALTER TABLE {RECOMMENDATION_TABLE_NAME} DROP COLUMN {column}"))


def create_model3_results_tables(region: str = "india") -> dict[str, list[str]]:
    ...
    with engine.begin() as conn:
        ...
        _drop_obsolete_before_rf_columns(conn)
        return {
...
def main() -> None:
    region = os.getenv("MODEL3_SCHEMA_REGION", "india")
    tables = create_model3_results_tables(region)
    expected = {
