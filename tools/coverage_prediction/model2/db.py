...
def fetch_baseline_rows(
    ...
    if bridge:
        try:
            ...
            df = bridge.get_rows("GetLteBaselineRows", params, limit=page_limit, progress_label="model2_baseline")
...
def fetch_geo_rows(
    ...
    if bridge:
        try:
            params = {"projectId": int(project_id), "region": str(region or "india").lower()}
            df = bridge.get_rows("GetLtePredictionGeoFeatures", params, limit=50000, progress_label="model2_geo")
...
def save_results(df: pd.DataFrame, region: str = "india") -> int:
    ...
    with get_engine(region).begin() as conn:
        ...
        out.to_sql(TABLE_NAME, con=conn, if_exists="append", index=False, method="multi", chunksize=2000)
    return int(len(out))
