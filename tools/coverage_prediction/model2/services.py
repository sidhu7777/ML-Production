...
DEFAULT_INPUT_EXCEL = ML_ROOT / "tools" / "coverage_prediction" / "model2" / "data" / "project_196_model2_demand_capacity_input.xlsx"
...
def _future_forecast_from_model_outputs(feature_df: pd.DataFrame, pred: pd.DataFrame) -> pd.DataFrame:
    ...
    demand_delta = (demand - demand.median()).clip(lower=0) * 0.25
    traffic_delta = (traffic - traffic.median()).clip(lower=0) * 0.35
    user_delta = (users - users.median()).clip(lower=0) * 0.4

    out = pd.DataFrame(index=feature_df.index)
    out["future_prb_utilization_pct"] = (current_prb + demand_delta + traffic_delta).clip(0, 100)
    out["future_rrc_utilization_pct"] = (current_rrc + user_delta + demand_delta * 0.5).clip(0, 100)
...
class LTEModel2DemandCapacityForecastService:
    ...
    def _update(self, job_id: str, status: str, progress: str, **extra: Any) -> None:
        ...
        print(f"[MODEL2][{job_id[:8]}] {status}: {progress}", flush=True)
    ...
    def _run_job(self, job_id: str, cfg: dict[str, Any]) -> None:
        ...
        try:
            ...
            model_run_id = str(cfg.get("model_run_id") or f"model2_{project_id}_{job_id[:8]}")
            ...
            self._update(job_id, "running", "Loading Model 2 cell input Excel")
            ...
            if cell_df.empty:
                raise ValueError(f"No Model 2 cell rows found for project_id={project_id}")

            self._update(job_id, "running", "Loading Model 2 weights")
            ...
            self._update(job_id, "running", "Building Model 2 demand/capacity feature frame")
            ...
            self._update(job_id, "running", f"Running Model 2 inference for {len(feature_df)} cells")
            ...
            out = pd.DataFrame(
                {
                    ...
                    "model_name": "model2_demand_capacity_forecast",
            ...
            self._update(job_id, "running", f"Saving {len(out)} Model 2 forecast rows to DB")
            rows_written = db.save_results(out, region=region)
            elapsed = (dt.datetime.utcnow() - started).total_seconds()
            self._update(
                job_id,
                "completed",
                "Model 2 demand/capacity forecast completed",
                project_id=project_id,
                baseline_job_id=str(resolved_baseline_job_id),
                model_run_id=model_run_id,
