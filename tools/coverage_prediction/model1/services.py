...
class LTEModel1CoveragePredictionService:
    ...
    def _run_job(self, job_id: str, cfg: dict[str, Any]) -> None:
        ...
        try:
            ...
            baseline_job_id = cfg.get("baseline_job_id")
            model_run_id = str(cfg.get("model_run_id") or f"model1_{project_id}_{job_id[:8]}")
            ...
            pred_df = feature_df[["grid_id", "grid_row", "grid_col", "grid_centroid_lat", "grid_centroid_lon"]].copy()

            self._update(job_id, "running", f"Running Model 1 inference for {len(feature_df)} grids")
            for target in TARGETS:
                pred_df[target] = bundle.models[target].predict(feature_df[x_cols])
            ...
            out = pd.DataFrame(
                {
                    ...
                    "model_name": "model1_coverage_xgboost",
            ...
            self._update(job_id, "running", f"Saving {len(out)} Model 1 rows to DB")
            rows_written = db.save_results(out, region=region)
            elapsed = (dt.datetime.utcnow() - started).total_seconds()
            self._update(
                job_id,
                "completed",
                "Model 1 coverage prediction completed",
